...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .btn-danger{background:linear-gradient(135deg,#dc3545 0%,#c82333 100%);box-shadow:0 4px 15px rgba(220,53,69,.3)}
    ...
    .spec-item{border:1px solid #ddd;border-radius:8px;padding:15px;margin-bottom:15px;background:#f9f9f9}
    .spec-item-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px}
    .spec-item-controls{display:grid;grid-template-columns:1fr 1fr 2fr;gap:10px;align-items:end}

    /* -------- XEM TRƯỚC TEM: kích thước thực tế -------- */
    ...
    /* KHỐI 5: grid động, hai cột bằng nhau, viền 1px */
    ...
    .b6-inner .company{font-size:calc(var(--h)*0.018);line-height:1.2;overflow:hidden;text-overflow:ellipsis}
    ...
    .b7-inner .phone{font-weight:800;font-size:calc(var(--h)*0.024);line-height:1.05;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    ...
    /* KHỐI 9: Made in + ngày có cùng cỡ chữ */
...
<body>
  <div class="container">
    <div class="header">
      <img src="data:image/svg+xml;base64," alt="Nextwaves Industries" class="logo" crossorigin="anonymous" onerror="this.style.display='none'">
    ...
    <div id="label-tab" class="tab-content">
      <div class="form-row">
        <div class="form-group">
          ...
          <select id="labelSize">
            <option value="70x100">70 x 100</option>
      ...
      <div class="form-row">
        <div class="form-group">
          <label for="productName">🏷️ <span data-lang-vi="Tên sản phẩm" data-lang-en="Product Name">Tên sản phẩm</span>:</label>
          <input type="text" id="productName" placeholder="VD: ĂNG TEN MẶT ĐẤT TUYẾN TÍNH">
        </div>
        <div class="form-group">
          <label for="technicalName">🔧 <span data-lang-vi="Tên kỹ thuật" data-lang-en="Technical Name">Tên kỹ thuật</span>:</label>
      ...
      <!-- Thông số kỹ thuật linh hoạt -->
      <div class="form-group">
        <label>⚙️ <span data-lang-vi="Thông số kỹ thuật" data-lang-en="Technical Specifications">Thông số kỹ thuật</span>:</label>
        <div id="specContainer">
          <!-- Sẽ được tạo động bằng JavaScript -->
        </div>
        <button type="button" class="btn" onclick="addSpecification()">➕ <span data-lang-vi="Thêm thông số" data-lang-en="Add Specification">Thêm thông số</span></button>
      </div>

      <div class="form-row">
        <div class="form-group">
          <label for="labelBarcodeText">📊 <span data-lang-vi="Mã vạch (EAN-13)" data-lang-en="Barcode (EAN-13)">Mã vạch (EAN-13)</span>:</label>
          <input type="text" id="labelBarcodeText" placeholder="VD: 8936236710036">
        </div>
        <div class="form-group">
          <label for="manufactureDate">📅 <span data-lang-vi="Ngày sản xuất" data-lang-en="Manufacture Date">Ngày sản xuất</span>:</label>
      ...
      <div class="form-row">
        <div class="form-group">
          <label for="labelBarcodeFormat">📋 <span data-lang-vi="Định dạng mã vạch" data-lang-en="Barcode Format">Định dạng mã vạch</span>:</label>
        ...
        <div class="form-group">
          <label for="labelBarcodeWidth">📏 <span data-lang-vi="Độ rộng thanh" data-lang-en="Bar Width">Độ rộng thanh</span>:</label>
      ...
      <div class="form-row">
        <div class="form-group">
          <label for="labelBarcodeHeight">📐 <span data-lang-vi="Chiều cao (px)" data-lang-en="Height (px)">Chiều cao (px)</span>:</label>
          <input type="number" id="labelBarcodeHeight" min="20" max="200" placeholder="100">
        </div>
        <div class="form-group">
          <label for="labelShowText">🔤 <span data-lang-vi="Hiển thị số" data-lang-en="Show Text">Hiển thị số</span>:</label>
          <div class="checkbox-group">
            <input type="checkbox" id="labelShowText" checked>
            <span data-lang-vi="Hiển thị số bên dưới mã vạch" data-lang-en="Show numbers below barcode">Hiển thị số bên dưới mã vạch</span>
          </div>
        </div>
      </div>

      <div style="text-align:center;">
        <button class="btn" onclick="generateLabel()">🎯 <span data-lang-vi="Tạo Tem Sản Phẩm" data-lang-en="Generate Product Label">Tạo Tem Sản Phẩm</span></button>
        <button class="btn" onclick="printLabel()">🖨️ <span data-lang-vi="In Tem" data-lang-en="Print Label">In Tem</span></button>
      </div>

      <div class="download-options">
        <h4>📥 <span data-lang-vi="Tải xuống PDF" data-lang-en="Download PDF">Tải xuống PDF</span></h4>
        <button class="btn btn-success" onclick="downloadLabelPDF()">📄 <span data-lang-vi="Tải xuống 1 tem PDF" data-lang-en="Download 1 label PDF">Tải xuống 1 tem PDF</span></button>
      </div>

      <div id="labelResult" class="result-section" style="display:none;">
        <h3>✅ <span data-lang-vi="Tem sản phẩm đã được tạo thành công!" data-lang-en="Product label created successfully!">Tem sản phẩm đã được tạo thành công!</span></h3>
  ...
  <script>
    ...
    const translations = {
      vi: {
        ...
        'Độ lợi': 'Độ lợi', 
        'VSWR': 'VSWR',
        'Nguồn đầu vào': 'Nguồn đầu vào',
        'company_info': 'Sản xuất bởi công ty TNHH Nextwaves Industries, 20/23 đường 35, Phường An Khánh, Thành Phố Hồ Chí Minh',
        'made_in': 'Made in Vietnam',
        'brand_name': 'NEXTWAVES INDUSTRIES'
      },
      en: {
        'Dải tần': 'Frequency range',
        'Độ lợi': 'Gain',
        'VSWR': 'VSWR', 
        'Nguồn đầu vào': 'Input Power',
        'company_info': 'Manufactured by Nextwaves Industries Co., Ltd, 20/23 Street 35, An Khanh Ward, Ho Chi Minh City',
        'made_in': 'Made in Vietnam',
        'brand_name': 'NEXTWAVES INDUSTRIES'
      }
    };
    ...
    function updateLanguage() {
      ...
      // Cập nhật tất cả các phần tử có data-lang
      document.querySelectorAll('[data-lang-vi]').forEach(el => {
        if (currentLanguage === 'en' && el.getAttribute('data-lang-en')) {
    ...
    function updatePlaceholders() {
      if (currentLanguage === 'en') {
        document.getElementById('productName').placeholder = "e.g.: LINEAR GROUND ANTENNA";
        document.getElementById('technicalName').placeholder = "e.g.: RA-UL-SFF-241401";
      } else {
        document.getElementById('productName').placeholder = "VD: ĂNG TEN MẶT ĐẤT TUYẾN TÍNH";
        document.getElementById('technicalName').placeholder = "VD: RA-UL-SFF-241401";
      }
    }
    ...
    function addSpecification() {
      ...
      specItem.innerHTML = `
        <div class="spec-item-header">
          <strong>Thông số ${specCount}</strong>
          <button type="button" class="btn btn-danger" onclick="removeSpecification(this)" style="padding:5px 10px;margin:0;">🗑️ Xóa</button>
        </div>
        <div class="spec-item-controls">
          <select class="spec-type">
            <option value="Dải tần">Dải tần</option>
            <option value="Độ lợi">Độ lợi</option>
            <option value="VSWR">VSWR</option>
            <option value="Nguồn đầu vào">Nguồn đầu vào</option>
          </select>
          <select class="spec-operator">
            <option value="">=</option>
            <option value="≤">≤</option>
            <option value="≥">≥</option>
            <option value="<"><</option>
          ...
          <input type="text" class="spec-value" placeholder="Nhập giá trị">
        </div>
      `;
    ...
    // Khởi tạo với 2 thông số mặc định
    function initializeSpecifications() {
      ...
      if (specItems[0]) {
        specItems[0].querySelector('.spec-type').value = 'Dải tần';
        specItems[0].querySelector('.spec-value').placeholder = 'VD: 900 - 930 MHz';
      }
      if (specItems[1]) {
        specItems[1].querySelector('.spec-type').value = 'Độ lợi';
        specItems[1].querySelector('.spec-value').placeholder = 'VD: 7.5 dBi';
      }
    }
    ...
    async function generateLabel() {
      ...
      document.querySelectorAll('.spec-item').forEach(item => {
        const type = item.querySelector('.spec-type').value;
        const operator = item.querySelector('.spec-operator').value;
        const value = item.querySelector('.spec-value').value.trim();
        if (value) {
          const translatedType = translations[currentLanguage][type] || type;
          specifications.push({
            type: translatedType,
      ...
      if (specifications.length < 2) {
        alert('Vui lòng nhập ít nhất 2 thông số kỹ thuật!');
        return;
      }
      ...
      try {
        ...
        b2.innerHTML = `<div class="b2-inner"><div class="brand">${translations[currentLanguage]['brand_name']}</div></div>`;
        ...
        // Tạo grid động cho thông số kỹ thuật với độ cao các dòng bằng nhau
        const b5 = document.createElement('div'); b5.className = 'b5';
        const rowCount = Math.ceil(specifications.length / 2);
        const gridRows = `repeat(${rowCount}, 1fr)`;
        
        let specGridHtml = '';
        for (let i = 0; i < specifications.length; i += 2) {
          const spec1 = specifications[i];
          const spec2 = specifications[i + 1];
          specGridHtml += `<div class="cell head">${spec1.type}</div><div class="cell">${spec1.value}</div>`;
          if (spec2) {
            specGridHtml += `<div class="cell head">${spec2.type}</div><div class="cell">${spec2.value}</div>`;
          } else {
            specGridHtml += `<div class="cell head"></div><div class="cell"></div>`;
          }
        }
        
        b5.innerHTML = `<div class="b5-inner">
          <div class="spec-grid" style="grid-template-rows: ${gridRows};">
            ${specGridHtml}
          </div>
        </div>`;

        const b6 = document.createElement('div'); b6.className = 'b6';
        b6.innerHTML = `<div class="b6-inner"><div class="company">${translations[currentLanguage]['company_info']}</div></div>`;
        ...
        b9.innerHTML = `<div class="b9-inner">
          <div class="madein">${translations[currentLanguage]['made_in']}</div>
          <span class="mfgdate">${dateDDMMYYYY}</span>
        </div>`;

        grid.append(b1, b2, b3, b4, b5, b6, b7, b8, b9);
        labelPreview.innerHTML = ''; 
        labelPreview.appendChild(grid);

        const exactH = size === '70x100' ? 500 : 200;
    ...
    function printLabel() {
      ...
      const dims = size === '70x100' ? '350px 500px' : '150px 200px';
