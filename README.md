...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    input[type="text"],input[type="number"],select{width:100%;padding:12px 16px;border:2px solid #e9ecef;border-radius:8px;font-size:1rem;transition:border-color .3s;background:#fff}
    input[type="text"]:focus,input[type="number"]:focus,select:focus{outline:none;border-color:#007bff;box-shadow:0 0 0 3px rgba(0,123,255,.1)}
    ...
    /* Khu xem trước tem theo đúng khổ mm */
    .label-preview{background:#fff;border:2px dashed #999;margin:20px auto;font-family:Arial,sans-serif;position:relative;display:block;overflow:hidden}
    .label-70x100{width:350mm;height:500mm}
    .label-30x40{width:150mm;height:200mm}

    /* 9 khối theo % chiều cao */
    .label-surface{position:relative;width:100%;height:100%}
    .block{position:absolute;left:10px;right:0;display:flex;align-items:center;justify-content:flex-start}
    .block-inner{width:100%;height:100%;padding:5px;display:flex;align-items:center;justify-content:flex-start;overflow:hidden}
    /* Căn giữa cho 1,2,8,9 */
    ...
    /* Bảng TSKT khối 5 (2 cột, căn trái) */
...
<body>
  ...
  <script>
    ...
    function generateBarcode(){
      ...
      try{
        JsBarcode("#barcodeDisplay",text,{format,width,height,displayValue:showText,background:"#ffffff",lineColor:"#000000",margin:10,fontSize:14,textAlign:"center",textPosition:"bottom"});
    ...
    function downloadBarcode(){
      ...
      if(!el||!el.innerHTML){alert('Vui lòng tạo mã vạch trước khi tải xuống!');return;}
      try{
        ...
        img.onload=function(){
          ...
          canvas.toBlob(function(blob){
            if(blob&&blob.size>0){
              ...
              a.download=`ma-vach-${(document.getElementById('barcodeInput').value||'barcode')}-${Date.now()}.png`;
              a.href=URL.createObjectURL(blob);document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(a.href);
    ...
    function generateLabelBarcode(text,format,width,height,showText){
      return new Promise((resolve,reject)=>{
        try{
          ...
          JsBarcode(canvas,text,{format,width,height,displayValue:showText,background:"#ffffff",lineColor:"#000000",margin:5,fontSize:12,textAlign:"center",textPosition:"bottom"});
    ...
    async function generateLabel(){
      ...
      const showText=true; // Khối 8 luôn có số ở dưới
      ...
      try{
        ...
        preview.innerHTML=`
          <div class="label-surface" id="labelSurface">
            ...
            <!-- 9: Made in Vietnam + Năm sản xuất -->
        ...
        applyBlocksLayout(); // áp layout 9 khối

        // Co chữ để vừa khối
        const maxMap={brandName:120,prodName:110,techName:100,companyInfo:70,phone:90,barcodeNumbers:60,madeIn:100,yearLine:80};
        Object.keys(maxMap).forEach(id=>{const el=document.getElementById(id);if(el)fitText(el,maxMap[id],8);});
    ...
    // Bố trí 9 khối: top 2%, gap 1% giữa các khối, bottom 2%
    function applyBlocksLayout(){
      const surface=document.getElementById('labelSurface');
      const H=surface.clientHeight;
      const topPad=0.02*H;   // 2%
      const gap=0.01*H;      // 1%
      // 12,3,4,4,24,5,4,25,7 = 88 (%)
      const heights=[12,3,4,4,24,5,4,25,7].map(p=>p/100*H);
    ...
    /* ====== Xuất PDF: chụp toàn bộ DOM và dán lên PDF theo khổ đã chọn ====== */
    ...
    /* Theo yêu cầu:
       - KHÔNG gán sự kiện input để tự sinh tem/mã vạch
       - KHÔNG tự động gọi generateLabel khi tải trang
       - KHÔNG auto-generate bất kỳ nội dung nào
    */
  </script>
</body>
</html>
